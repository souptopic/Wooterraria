...
namespace Wooting
{
    ...
    {
        private const string sdkDLL = "wooting-rgb-sdk64";
        ...
        {
            string dllPath = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, sdkDLL + ".dll");

            if (!System.IO.File.Exists(dllPath))
            {
                string url = "https://github.com/WootingKb/wooting-rgb-sdk/releases/download/v1.7.0/wooting-rgb-sdk-v1.7.0-win-x64.zip";
                string tempZipPath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "wooting-rgb-sdk.zip");
                string tempExtractPath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "wooting-rgb-sdk");
                ...
                using (var client = new System.Net.Http.HttpClient())
                {
                    try
                    {
                        ...
                        System.IO.File.WriteAllBytes(tempZipPath, content);
                        System.IO.Compression.ZipFile.ExtractToDirectory(tempZipPath, tempExtractPath);
                        System.IO.File.Copy(System.IO.Path.Combine(tempExtractPath, "wooting-rgb-sdk64.dll"), dllPath, true);
                    }
                    catch (Exception ex)
                    {
                        mod.Logger.Error($"Failed to download or extract DLL: {ex.Message}");
                        return;
                    }
                    finally
                    {
                        ...
                        if (System.IO.Directory.Exists(tempExtractPath))
            ...
            {
                mod.Logger.Error($"Failed to load DLL from: {dllPath}");
                return;
            }
            mod.Logger.Info($"Successfully loaded DLL from: {dllPath}");


            if (!IsConnected())
            {
                mod.Logger.Warn("No Wooting keyboard connected.");
                return;
            }
            ...
            for (byte i = 0; i < count; i++)
            {
                ...
                mod.Logger.Info($"Found device: Connected: {device.Connected}, Model: {device.Model}, Type: {device.DeviceType}, Max Rows: {device.MaxRows}, Max Cols: {device.MaxColumns}, Max Keycode: {device.KeycodeLimit}");
                infos[i] = device;
            }
        }
